...
export async function getAnalyticsData(range = "month") {
  ...
  const onTimeRate = totalEmployees > 0 && (totalPresent + totalLate) > 0
  ...
  const departmentData = await Promise.all(
    departments.map(async (dept) => {
      ...
      const totalDepts = deptDaysPresent + deptDaysAbsent + deptDaysLate;
      const attendance = totalDepts > 0
        ? Math.round((deptDaysPresent / totalDepts) * 100)
        : 0;
      
      // Calculate efficiency based on actual on-time arrivals
      const onTimePercentage = (deptDaysPresent + deptDaysLate) > 0
        ? Math.round((deptDaysPresent / (deptDaysPresent + deptDaysLate)) * 100)
        : 0;
  ...
  return {
    kpis: {
      ...
      hoursProcessed: Math.floor(totalPresent * 8 + totalLate * 4),
