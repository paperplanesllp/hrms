...
router.post("/", requireAuth, requireRole(ROLES.HR, ROLES.ADMIN), uploadNews.single("image"), postNews);
...
router.post("/admin/cleanup-images", requireAuth, requireRole(ROLES.HR, ROLES.ADMIN), cleanupImages);

// Get single news detail
router.get("/:id", requireAuth, getNewsDetail);

// HR only: update/delete specific news
router.patch("/:id", requireAuth, requireRole(ROLES.HR, ROLES.ADMIN), uploadNews.single("image"), patchNews);
router.delete("/:id", requireAuth, requireRole(ROLES.HR, ROLES.ADMIN), removeNews);
